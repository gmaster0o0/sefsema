...
export default function RecipeManager({ recipes }: RecipeManagerProps) {
  ...
  const allIngredients = useMemo(() => getAllIngredients(localRecipes), [localRecipes]);
  ...
  useEffect(() => {
    const current = localRecipes.find((recipe) => recipe.id === editingId);
    setEditingTags(current?.tags ?? []);
  }, [editingId, localRecipes]);
  ...
  let visibleRecipes = localRecipes;
  ...
  const handleDelete = async (recipeId: string) => {
    // Elmentsuk az eredeti listat rollback-hez
    const previousRecipes = localRecipes;

    // Optimista update: azonnal eltavolitjuk a receptet a listabol
    setLocalRecipes((prev) => prev.filter((r) => r.id !== recipeId));
    setError(null);

    try {
      // Server action meghivasa
      const formData = new FormData();
      formData.append("id", recipeId);
      await deleteRecipeAction(formData);

      // Frissitjuk a cache-t
      router.refresh();
    } catch (err) {
      // ROLLBACK: Visszaallitjuk az eredeti listat
  ...
  const mainContent = (
    <>
      {editingId ? (
        <div className="mt-6 rounded-2xl border border-black/10 bg-zinc-50 p-6">
          ...
          <form action={handleUpdate} className="grid gap-4 md:grid-cols-2">
            ...
            {localRecipes.find((r) => r.id === editingId)?.imageUrl && (
              <div className="relative h-48 overflow-hidden rounded-lg md:col-span-2">
                <Image
                  src={localRecipes.find((r) => r.id === editingId)!.imageUrl}
                  alt="Preview"
                  fill
            ...
            <label className="grid gap-1 text-sm md:col-span-2">
              ...
              <input
                name="title"
                defaultValue={localRecipes.find((r) => r.id === editingId)?.title}
            ...
            <label className="grid gap-1 text-sm md:col-span-2">
              ...
              <input
                name="imageUrl"
                type="url"
                defaultValue={localRecipes.find((r) => r.id === editingId)?.imageUrl}
                className="rounded-lg border border-black/10 px-3 py-2"
              />
            </label>
            <label className="grid gap-1 text-sm md:col-span-2">
              ...
              <textarea
                name="ingredients"
                defaultValue={localRecipes.find((r) => r.id === editingId)?.ingredients.join("\n")}
            ...
            <label className="grid gap-1 text-sm md:col-span-2">
              ...
              <textarea
                name="preparation"
                defaultValue={localRecipes.find((r) => r.id === editingId)?.preparation}
                className="min-h-32 rounded-lg border border-black/10 px-3 py-2"
                required
              />
            </label>
            ...
            <label className="flex items-center gap-2 text-sm md:col-span-2">
              <input
                name="isPublic"
                type="checkbox"
                defaultChecked={localRecipes.find((r) => r.id === editingId)?.isPublic}
                className="h-4 w-4"
              />
      ...
      {visibleRecipes.length === 0 ? (
        <p className="mt-6 text-sm text-zinc-500">Nincs találat a kiválasztott szűrőkre.</p>
      ) : (
  ...
  return (
    <section className="rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
      <h2 className="text-lg font-semibold">Saját receptek</h2>
      <p className="mt-1 text-sm text-zinc-500">Szerkeszd vagy töröld saját receptjeidet.</p>
      ...
      {localRecipes.length === 0 ? (
        <p className="mt-6 text-sm text-zinc-500">Még nincs saját recepted.</p>
      ) : (
        <>
